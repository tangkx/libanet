#include "libanet/base/ProcessInfo.h"
#include "libanet/base/CurrentThread.h"
#include "libanet/base/FileUtil.h"
...
int
fdDirFilter(const struct dirent *d) {
...
int
taskDirFilter(const struct dirent *d) {
	if (::isdigit(d->d_name[0])) {
...
int
scanDir(const char *dirpath, int (*filter)(const struct dirent *)) {
...
using namespace libanet::detail;
...
StringPiece
ProcessInfo::procname(const string &stat) {
	StringPiece name;
	size_t lp = stat.find('(');
	size_t rp = stat.find(')');
