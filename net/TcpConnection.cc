...
#include "libanet/base/WeakCallback.h"
...
void
libanet::net::defaultConnectionCallback(const TcpConnectionPtr &conn) {
...
void
libanet::net::defaultMessageCallback(const TcpConnectionPtr &, Buffer *buf,
									 Timestamp) {
	buf->retrieveAll();
}
...
: loop_(CHECK_NOTNULL(loop)), name_(name), state_(kConnecting), reading_(true),
